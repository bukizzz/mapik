// Paket proxy pruža server visokih performansi za OpenAI proxy sa više ključeva
package proxy
...
// ProxyServer predstavlja proxy server
...
// NewProxyServer kreira novi proxy server
func NewProxyServer(
	keyProvider *keypool.KeyProvider,
	groupManager *services.GroupManager,
	settingsManager *config.SystemSettingsManager,
...
// HandleProxy je glavna ulazna tačka za proxy zahteve, refaktorisana na osnovu stabilne .bak logike.
...
// executeRequestWithRetry je osnovna rekurzivna funkcija za obradu zahteva i ponovnih pokušaja.
func (ps *ProxyServer) executeRequestWithRetry(
	...
	// Jedinstveno rukovanje greškama za ponovne pokušaje.
	if err != nil || (resp != nil && resp.StatusCode >= 400) {
		...
		if err != nil {
			...
			if readErr != nil {
				logrus.Errorf("Nije uspelo čitanje tela greške: %v", readErr)
				errorBody = []byte("Nije uspelo čitanje tela greške")
			}

			errorBody = handleGzipCompression(resp, errorBody)
			errorMessage = string(errorBody)
			parsedError = app_errors.ParseUpstreamError(errorBody)
			logrus.Debugf("Zahtev nije uspeo sa statusom %d (pokušaj %d/%d) za ključ %s. Parsirana greška: %s", statusCode, retryCount+1, cfg.MaxRetries, utils.MaskAPIKey(apiKey.KeyValue), parsedError)
		}
	...
	// ps.keyProvider.UpdateStatus(apiKey, group, true) // Zahtev je uspeo, ne resetuje se broj uspešnih pokušaja, smanjuje se potrošnja I/O
	logrus.Debugf("Zahtev za grupu %s uspeo je u pokušaju %d sa ključem %s", group.Name, retryCount+1, utils.MaskAPIKey(apiKey.KeyValue))
...
// logRequest je pomoćna funkcija za kreiranje i beleženje loga zahteva.
